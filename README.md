...
- [ ] What problem does the context API help solve?
  Getting state anywhere in an app without having to pass it down long trees of components.
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  Actions: Information sent to the reducer that determines how state is changed.
  Reducers: A function that changes state based on the action it is sent.
  Store: Where the apps global state is stored, the store's state can only be changed by reducers, it is not changed directly in every component.
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
  Application state is global and component state is local. Application state should be used if state is needed by many components and their children on different levels of the app component should be used if state is only needed by a parents and its children.
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
  It allows for action creators to return functions instead of just an aciton.
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
  Redux, I like having all of the global state and the logic for changing it in one place instead of spread all over the application.
